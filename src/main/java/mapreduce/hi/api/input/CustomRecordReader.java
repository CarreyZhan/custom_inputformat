...
public class CustomRecordReader extends RecordReader<LongWritable, Text>{
    ...
    public CustomRecordReader(byte[] recordDelimiterBytes) {
        this.recordDelimiterBytes = recordDelimiterBytes;
        LOG.info("using delimiter:" + recordDelimiterBytes);
        System.out.println("print using delimiter:" + recordDelimiterBytes);
    }
    ...
    public boolean nextKeyValue() throws IOException, InterruptedException {
        ...
        if (this.value == null) {
          this.value = new Text(new byte[DEFAULT_BUFFER_SIZE]);
        }
        int newSize = 0;

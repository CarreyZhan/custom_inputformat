...
public class CustomReader {
   ...
   */
  public int readObject(Text buff, long maxBytesToConsume, int maxRecordLength) throws IOException {
    ...
    */
    ...
    do {
      ...
      if (bufferPosn >= bufferLength) {// initailly read byte[] from stream
        ...
        if (bufferLength <= 0) {
            buff.append(recordDelimiterBytes, 0, ambiguousByteCount);
          break; // EOF
        }
      }
      /*
      ...
      if (appendLength > 0) {
        ...
        if(buffer != null){
            buff.append(buffer, startPosn, appendLength);
